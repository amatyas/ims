...
class TbEditableField extends CWidget {
    ...
    public function registerClientScript() {
        ...
        if ($this->parentid) {
            Yii::app()->getClientScript()->registerScript(
                    __CLASS__ . '#' . $this->parentid . '-' . $this->id, $script
            );
        } else {
    ...
    public function registerAssets() {
        ...
        Yii::app()->bootstrap->registerAssetJs(
                'bootstrap-editable' . (!YII_DEBUG ? '.min' : '') . '.js', CClientScript::POS_HEAD
        );

        if ($this->type == 'date' || $this->type == 'combodate') {
            /** @var $widget TbDatePicker */
            $widget = Yii::app()->widgetFactory->createWidget(
                    $this->getOwner(), 'bootstrap.widgets.TbDatePicker', array('options' => $this->options['datepicker'])
            );
