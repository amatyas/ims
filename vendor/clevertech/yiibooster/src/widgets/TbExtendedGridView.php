...
class TbExtendedGridView extends TbGridView {
    ...
    public function init() {
        ...
        if (preg_match(
                        '/extendedsummary/i', $this->template
    ...
    public function renderKeys() {
        ...
        foreach ($data as $d) {
            echo CHtml::tag(
                    'span', array('data-order' => $this->getAttribute($d, $this->sortableAttribute)), CHtml::encode($this->getPrimaryKey($d))
            );
        }
        echo "</div>\n";
        return true;
    ...
    public function renderChart() {
        ...
        if (!isset($this->chartOptions['data']['series'])) {
            throw new CException(Yii::t(
                    'zii', 'You need to set the "series" attribute in order to render a chart'
            ));
        }
            ...
            for ($row = 0; $row < $count; ++$row) {
                ...
                if (!is_null($column) && $column->value !== null) {
                    $seriesData[$cnt]['data'][] = $this->evaluateExpression(
                            $column->value, array('data' => $data[$row], 'row' => $row)
                    );
                } else {
        ...
        $options = CMap::mergeArray(
                        $this->chartOptions['config'], array('series' => $seriesData)
        );
        ...
        if ($this->sortableRows) {
            ...
            if ($this->sortableAjaxSave && $this->sortableAction !== null) {
                $sortableAction = Yii::app()->createUrl(
                        $this->sortableAction, array('sortableAttribute' => $this->sortableAttribute)
                );
            } else {
                $sortableAction = '';
            }
					...
					options.success = function(data)
					{
						if (options.realsuccess) {
							...
							' . (count($this->componentsAfterAjaxUpdate) ? implode(
                                PHP_EOL, $this->componentsAfterAjaxUpdate
                        ) : '') . '
						}
					}
				}
    ...
    protected function getSummaryOperationInstance($name, $config) {
        if (!isset($config['class'])) {
            throw new CException(Yii::t(
                    'zii', 'Column summary configuration must be an array containing a "type" element.'
            ));
        }

        if (!in_array($config['class'], $this->extendedSummaryOperations)) {
            throw new CException(Yii::t(
                    'zii', '"{operation}" is an unsupported class operation.', array('{operation}' => $config['class'])
            ));
        }
...
abstract class TbOperation extends CWidget {
    ...
    public function init() {
        if (null == $this->column) {
            throw new CException(Yii::t(
                    'zii', '"{attribute}" attribute must be defined', array('{attribute}' => 'column')
            ));
        }
    }
...
class TbSumOperation extends TbOperation {
    ...
    public function init() {
        ...
        if (!in_array($this->column->type, $this->supportedTypes)) {
            throw new CException(Yii::t(
                    'zii', 'Unsupported column type. Supported column types are: "{types}"', array(
...
class TbCountOfTypeOperation extends TbOperation {
    ...
    public function init() {
        if (empty($this->types)) {
            throw new CException(Yii::t(
                    'zii', '"{attribute}" attribute must be defined', array('{attribute}' => 'types')
            ));
        }
    ...
    public function displaySummary() {
        ...
        foreach ($this->types as $type) {
            ...
            $typesResults[] = strtr(
                    $this->typeTemplate, array('{label}' => $type['label'], '{value}' => $type['value'])
            );
        }
