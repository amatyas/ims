...
class Bootstrap extends CApplicationComponent {
    ...
    public function init() {
        ...
        $this->packages = CMap::mergeArray(
                        require(Yii::getPathOfAlias('bootstrap.components') . DIRECTORY_SEPARATOR . 'packages.php'), $this->packages
        );
    ...
    public function registerDateRangePlugin($selector, $options = array(), $callback = null) {
        Yii::app()->getClientScript()->registerScript(
                $this->getUniqueScriptId(), '$("' . $selector . '").daterangepicker(' . CJavaScript::encode($options) . ($callback ? ', ' . CJavaScript::encode($callback) : '') . ');'
        );
    }
    ...
    protected function registerPlugin($name, $selector = null, $options = array(), $defaultSelector = null) {
        ...
        if (isset($selector)) {
            ...
            Yii::app()->getClientScript()->registerScript(
                    $this->getUniqueScriptId(), "jQuery('{$selector}').{$name}({$options});"
            );
        }
    }
    ...
    public function getAssetsUrl() {
        if (isset($this->_assetsUrl)) {
            ...
            return $this->_assetsUrl = Yii::app()->getAssetManager()->publish(
                    Yii::getPathOfAlias('bootstrap.assets'), false, -1, $this->forceCopyAssets
            );
        }
    }
